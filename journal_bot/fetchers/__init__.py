...
def build_fetcher(
    ...
    if jc.type == "openalex":
        return OpenAlexFetcher(jc, since_year=since_year, end_year=end_year)
    if jc.type == "dce":
        return DCEFetcher(jc, since_year=since_year, end_year=end_year)
    if jc.type == "custom":
        return ConfigurableFetcher(jc, since_year=since_year, end_year=end_year)
